...
public class YeononAuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if (header == null || !header.startsWith("Basic ")) {
            throw new UnapprovedClientAuthenticationException("没有Basic验证");
        }

        String[] tokens = extractAndDecodeHeader(header, request);

        if (tokens.length != 2) {
            throw new UnapprovedClientAuthenticationException("Basic 解析失败");
        }
        ...
        if (!StringUtils.equals(clientSecret,clientDetails.getClientSecret())) {
            throw new UnapprovedClientAuthenticationException("clientSecRet对应的配置信息不正确 " + clientId);
        }
